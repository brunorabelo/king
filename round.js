...
export function createRound(
  ...
  function checkRoundIsFinished() {
    ...
    const roundFInishedRules = {
      ...
      5: defaultRule,
      6: (x) =>
        x.filter((x) => x.rank === "k" && x.suit === "heart").length >= 1,
  ...
  function updatePontuation() {
    ...
    const roundPontuationRules = {
      ...
      2: (x) =>
        x.reduce(
          (acc, curr) => acc + curr.filter((x) => x.suit === "heart").length,
          0
        ) * -20,
      3: (x) =>
        x.reduce(
          (acc, curr) =>
            acc + curr.filter((x) => x.rank === "j" || x.rank === "k").length,
          0
        ) * -30,
      4: (x) =>
        x.reduce(
      ...
      6: (x) =>
        x.reduce(
          (acc, curr) =>
            acc +
            curr.filter((x) => x.rank === "k" && x.suit === "heart").length,
          0
        ) * -160,
      7: (x) => x.length * +20,
      8: (x) => x.length * +20,
